...
// Update the version number in the CSS and JavaScript URLs
document.addEventListener("DOMContentLoaded", function () {
  // Get all CSS links
  const cssLinks = document.querySelectorAll('link[rel="stylesheet"]');
  // Update the href attribute of each CSS link
  cssLinks.forEach(function (link) {
    const href = link.getAttribute("href");
    link.setAttribute("href", href + "?ver=" + getVersion());
  });

  // Get all JavaScript scripts
  const jsScripts = document.querySelectorAll("script[src]");
  // Update the src attribute of each JavaScript script
  jsScripts.forEach(function (script) {
    const src = script.getAttribute("src");
    script.setAttribute("src", src + "?ver=" + getVersion());
  });
});
